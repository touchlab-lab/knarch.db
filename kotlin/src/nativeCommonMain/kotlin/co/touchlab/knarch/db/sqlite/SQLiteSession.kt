...
class SQLiteSession(private val mConnection: SQLiteConnection) {
    ...
    private fun beginTransactionUnchecked(transactionMode:Int,
        ...
        transaction.mParent.value = mTransactionStack
    ...
    fun setTransactionSuccessful() {
        withLock {
            ...
            throwIfTransactionMarkedSuccessful()
            mConnection.getTransaction()!!.mMarkedSuccessful.value = true
        }
    }
    ...
    private fun endTransactionUnchecked(yielding:Boolean) {
        ...
        var successful = (top!!.mMarkedSuccessful.value || yielding) && !top.mChildFailed.value
        ...
        mConnection.putTransaction(top.mParent.value)
        ...
        if (transStack != null)
        {
            if (!successful)
            {
                transStack.mChildFailed.value = true
            }
        }
        else
        {
    ...
    private fun throwIfTransactionMarkedSuccessful() {
        val mTransactionStack= mConnection.getTransaction()
        if (mTransactionStack != null && mTransactionStack.mMarkedSuccessful.value)
        {
    ...
    private fun obtainTransaction(mode:Int, listener:SQLiteTransactionListener?):Transaction = Transaction(mMode = mode, mListener = listener)

    internal data class Transaction(
            val mParent:AtomicReference<Transaction?> = AtomicReference(null),
            val mMode:Int = 0,
            val mListener:SQLiteTransactionListener? = null,
            val mMarkedSuccessful:AtomicBoolean = AtomicBoolean(false),
            val mChildFailed:AtomicBoolean = AtomicBoolean(false)
    )
