...
import co.touchlab.kite.Log
import co.touchlab.kite.db.ContentValues
import co.touchlab.kite.db.Cursor
import co.touchlab.kite.db.DatabaseErrorHandler
import co.touchlab.kite.db.SQLException
...
class SQLiteDatabase private constructor(path: String, openFlags: Int, cursorFactory: CursorFactory?, errorHandler: DatabaseErrorHandler?) : SQLiteClosable() {
    ...
    companion object {
         ...
         */
        fun openDatabase(path:String , factory:CursorFactory , flags:Int, errorHandler:DatabaseErrorHandler? = null):SQLiteDatabase {
    ...
    val sqliteSession = createSession();
    ...
    // INVARIANT: Immutable.
    private val mErrorHandler:DatabaseErrorHandler

    // Warns if the database is finalized without being closed properly.
    // INVARIANT: Guarded by mLock.
    private val mCloseGuardLocked = CloseGuard.get()
    ...
    // INVARIANT: Guarded by mLock.
    private var mConnectionPoolLocked:SQLiteConnectionPool?

    // True if the database has attached databases.
    // INVARIANT: Guarded by mLock.
    private var mHasAttachedDbsLocked = false
    ...
    private fun dispose(finalized:Boolean) {
        val pool = mConnectionPoolLocked
     ...
     * @return The label.
     */
    fun getLabel() = mConfigurationLocked.label
    ...
    fun createSession():SQLiteSession {
        ...
        return SQLiteSession(mConnectionPoolLocked);
    }
     ...
     */
    fun getThreadDefaultConnectionFlags(readOnly:Boolean):Int = if(readOnly){SQLiteConnectionPool.CONNECTION_FLAG_READ_ONLY}
        else{SQLiteConnectionPool.CONNECTION_FLAG_PRIMARY_CONNECTION_AFFINITY}
    ...
    fun reopenReadWrite() {
        ...
        val oldOpenFlags = mConfigurationLocked.openFlags;
        mConfigurationLocked.openFlags = (mConfigurationLocked.openFlags.and(OPEN_READ_MASK.inv())).or(OPEN_READWRITE)
        try {
...
//            assert(mConnectionPoolLocked == null)
     ...
     */
    fun getVersion():Int = ((Long) DatabaseUtils.longForQuery(this, "PRAGMA user_version;", null)).intValue()
    ...
    fun delete(table:String, whereClause:String, whereArgs:Array<String>):Int {
        ...
        try
        {
            val statement = SQLiteStatement(this, ("DELETE FROM " + table +
                    (if (!TextUtils.isEmpty(whereClause)) " WHERE " + whereClause else "")), whereArgs)
            try
            {
    ...
    private fun isReadOnlyLocked():Boolean {
        return (mConfigurationLocked.openFlags and OPEN_READ_MASK) === OPEN_READONLY
    }
    /**
    ...
    fun setLocale(locale:Locale) {
        ...
        synchronized (mLock) {
            throwIfNotOpenLocked()
            val oldLocale = mConfigurationLocked.locale
    ...
    fun enableWriteAheadLogging():Boolean {
        synchronized (mLock) {
            throwIfNotOpenLocked()
            if ((mConfigurationLocked.openFlags and ENABLE_WRITE_AHEAD_LOGGING) !== 0)
            {
                return true
            }
            if (isReadOnlyLocked())
    ...
    fun isWriteAheadLoggingEnabled():Boolean {
        synchronized (mLock) {
            throwIfNotOpenLocked()
            return (mConfigurationLocked.openFlags and ENABLE_WRITE_AHEAD_LOGGING) !== 0
        }
    }
    /**
     ...
     * Used by bug report.
     */
    fun getDbStats():ArrayList<DbStats> {
        val dbStatsList = ArrayList<DbStats>()
        for (db in getActiveDatabases())
        {
    ...
    private fun getActiveDatabases():ArrayList<SQLiteDatabase> {
        val databases = ArrayList<SQLiteDatabase>()
     ...
     * Used by bug report.
     */
    fun dumpAll(printer:Printer, verbose:Boolean) {
        for (db in getActiveDatabases())
        {
            db.dump(printer, verbose)
        }
    }
    private fun dump(printer:Printer, verbose:Boolean) {
        synchronized (mLock) {
            if (mConnectionPoolLocked != null)
            {
                printer.println("")
    ...
    fun isDatabaseIntegrityOk():Boolean {
        ...
        try
        {
            val attachedDbs:List<Pair<String, String>> = null
            try
            {
                attachedDbs = getAttachedDbs()
                if (attachedDbs == null)
                {
                    throw IllegalStateException(("databaselist for: " + getPath() + " couldn't " +
                            "be retrieved. probably because the database is closed"))
                }
            }
            catch (e:SQLiteException) {
                // can't get attachedDb list. do integrity check on the main database
                attachedDbs = ArrayList<Pair<String, String>>()
                attachedDbs.add(Pair<String, String>("main", getPath()))
            }
            for (i in 0 until attachedDbs.size())
            {
                val p = attachedDbs.get(i)
                val prog:SQLiteStatement = null
                try
                {
    ...
    private fun throwIfNotOpenLocked() {
        if (mConnectionPoolLocked == null)
        {
            throw IllegalStateException(("The database '" + mConfigurationLocked.label
                    + "' is not open."))
        }
    }
    ...
    init {
        mErrorHandler = errorHandler ?: DefaultDatabaseErrorHandler()
        mConfigurationLocked = SQLiteDatabaseConfiguration(path, openFlags)
    }
}